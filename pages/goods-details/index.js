...
Page({
  ...
  onLoad: function (e) {
    ...
    HalgaiAPI.goodsDetail(e.id).then(function (res) {
      var selectSizeTemp = "";
  ...
  buyNow: function (e) {
    ...
    if (shoptype == 'toPingtuan') {
      if (this.data.pingtuanopenid) {
        ...
        HalgaiAPI.pingtuanOpen(that.data.goodsDetail.basicInfo.id, wx.getStorageSync('token')).then(function (res) {
          if (res.code != 0) {
            wx.showToast({
              title: res.msg,
              icon: 'none',
  ...
  pingtuanList: function (goodsId) {
    var that = this;
    HalgaiAPI.pingtuanList(goodsId).then(function (res) {
      if (res.code == 0) {
        that.setData({
  ...
  goKanjia: function () {
    ...
    HalgaiAPI.kanjiaJoin(that.data.curGoodsKanjia.id, wx.getStorageSync('token')).then(function (res) {
      if (res.code == 0) {
        wx.navigateTo({
